 ...
 * has room for another allocation. This includes chunks that appear full from
 * the next available pointer but actually have free space If all chunks are
 * full, a new chunk will be initialized and added to the pool.
 */
chunk_t * get_nonfull_chunk(smallobj_pool_t *pool) {
    ...
    while (chunk != NULL) {
        ...
        /*
         * If a chunk doesn't have normal allocation space see if anything has
         * been freed inside so that it can be filled
         */
        else if (chunk->num_freed > 0){
          chunk->num_freed--;
          break;
        }
...
void * alloc_object_from_chunk(chunk_t *chunk) {
    ...
    // If a chunk hasn't been fully allocated then allocate memory normally

    if (chunk->next_available < chunk->pool->objects_per_chunk) {
      obj = chunk->mem + chunk->next_available * chunk->pool->objsize;
      chunk->next_available++;
    }
    /*
     * If a chunk doesn't have normal allocation space fill in the free space
     * within the chunk by iterating through the chunk until a free space is
     * found via the garbage value 0xEE
     */
    else {
      for(int i = 0; i < chunk->pool->objects_per_chunk; i++) {
          if (*(uint8_t *)((chunk->mem) + i * chunk->pool->objsize) == 0xEE){
            obj = ((chunk->mem) + i * chunk->pool->objsize);
            break;
        }
      }
    }
...
void so_free(smallobj_pool_t *pool, void *obj) {
    ...
    if (chunk == NULL) {
        fprintf(stderr, "ERROR:  Pointer %p isn't from pool %p\n", obj, pool);
        abort();
    }
