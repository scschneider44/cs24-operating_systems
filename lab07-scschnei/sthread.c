...
void sthread_start(int timer) {
    /* __sthread_start calls __sthread_scheduler. Thus, to prevent multiple
     * entrances to the scheduler we must lock it so that we can maintain
     * mutual exclusion
     */
    __sthread_lock();
    if (timer)
        start_timer();
    __sthread_start();
    /* __sthread_scheduler returns after start and since it has been exited we
     * can unlock the scheduler
     */
    __sthread_unlock();
}
 ...
 */
void __sthread_finish(void) {
    /* __sthread_switch calls __sthread_scheduler. Thus, to prevent multiple
     * entrances to the scheduler we must lock it so that we can maintain
     * mutual exclusion
     */
    __sthread_lock();
...
void sthread_unblock(Thread *threadp) {
    ...
    /* This function changes the queue. To protect the queue from being accessed
     * simultaneously, we must lock the scheduler. This prevents mangled queues
     */
    __sthread_lock();
    ...
    threadp->state = ThreadReady;
    enqueue_thread(threadp);
    /* After we are done manipulating the queue scheduler can alter it so we
     * can unlock
     */
    __sthread_unlock();
}
